    ...
    public class ApiControllerBase : ApiController
    {
        ...
        protected HttpResponseMessage CreateHttpResponse(HttpRequestMessage requestMessage, Func<HttpResponseMessage> func)
        {
            ...
            catch (DbEntityValidationException ex)
            {
                ...
                {
                    Trace.WriteLine($"Entity of type \"{eve.Entry.Entity.GetType().Name}\" in state \"{eve.Entry.Entity.GetType().Name}\" has the following validation error.");
            ...
            {
                LogError(dbEx);
                respone = requestMessage.CreateResponse(HttpStatusCode.BadRequest, dbEx.Message);
            }
            catch (Exception ex)
            {
                LogError(ex);
